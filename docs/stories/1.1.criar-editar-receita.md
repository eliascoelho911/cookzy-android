...
## Tasks / Subtasks
- [ ] Data Model (Room) (AC: 3)
  - [ ] Atualizar `IngredientEntity` para conter apenas `id`, `recipeId`, `rawText` (e `position` se necessário).
  - [ ] Remover `name/quantity/unit` de todas as camadas (entity, DTO, domain). Usar `fallbackToDestructiveMigration()` (app ainda não produtivo).
- [ ] DAO/Repository (AC: 3)
  - [ ] Ajustar DAOs e repositório para o novo esquema minimalista baseado em `rawText`.
  - [ ] Remover mapeamentos/acessos a campos antigos.
- [ ] Derivação de Quantidade (AC: 4)
  - [ ] Implementar utilitário puro `deriveQuantity(rawText: String): BigDecimal?` (ou equivalente) sem persistência.
  - [ ] Recalcular on-demand ao fornecer dados para UI/uso de domínio; retorno nulo quando não inferível.
- [ ] UI — Editor (AC: 1, 2)
  - [ ] Ingredientes: trocar para lista de linhas de texto (um campo por item); remover campo de observações.
  - [ ] Preparo: campo único multilinha.
  - [ ] Manter validação de título e mensagens de erro acessíveis.
- [ ] UI — Detalhe (AC: 5)
  - [ ] Exibir ingredientes a partir de `rawText` por linha.
- [ ] Navegação (AC: 5)
  - [ ] Manter fluxo salvar→Detalhe; cancelar→voltar.
- [ ] Testes (AC: 2, 3, 4, 5)
  - [ ] DAO: validar persistência apenas de `rawText` e ordem.
  - [ ] Utilitário de derivação: cobrir casos representativos e ambiguidades.
  - [ ] ViewModel: validação de título e integração com derivação on-demand.
  - [ ] Fluxo: verificação de navegação em salvar/cancelar.
...
- UI/Estado: Compose + ViewModel, com contratos de Estado/Evento/Efeito (docs/front-end/03-arquitetura-componentes.md). Editor com lista de linhas para ingredientes e campo único para preparo.
...
- Escopo desta história
  ...
  - Derivação: on-demand pela função `deriveQuantity(rawText: String): BigDecimal?`.

#### Ready for Dev (Checklist)
- [ ] Habilitar `fallbackToDestructiveMigration()` no builder do Room.
- [ ] Remover referências a `name/quantity/unit` de entity/DTO/domain/DAO/Repo.
- [ ] Introduzir utilitário `deriveQuantity(rawText: String)` sem persistência.
- [ ] Atualizar Editor/Detalhe para consumir apenas `rawText` e derivação on-demand.
- [ ] Atualizar módulos do Koin para quaisquer novos provedores/VMs afetados.
...
- ViewModel/UI: validação de título obrigatório e fluxo salvar/cancelar.

#### AC → Validação
- AC1 (Editor simplificado): teste de UI/Compose verificando presença dos campos (título, lista de ingredientes por linha, preparo multilinha) e interação básica.
