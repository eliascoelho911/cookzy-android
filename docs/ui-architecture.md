...
|---|---|---|---|---|---|
| AppTopBar | Barra superior com título/ações (suporta busca inline opcional) | `title: String`, `onBack: (() -> Unit)?`, `actions: @Composable RowScope.() -> Unit`, `search: SearchUi?` | padrão, scrolled, search-active (substitui título) | botão voltar com label; ordem de foco; campo de busca com hint/clear; com busca ativa recomenda-se scrollBehavior pinned | padrão; com back; com ações; com busca |
| RecentRecipesCarousel | Carrossel de receitas recentes | `items: List<Recipe>`, `onClick(Recipe)`, `peekDp: Dp` | vazio | anunciar “deslize p/ ver mais”; foco previsível; usa RecipeCard(Tile) | vazio; 1 item; vários |
...
| IconToggle (genérico) | Controle segmentado 2 estados (ex.: List ↔ Tile) | `options(2)`, `selectedIndex`, `onSelect(Int)` | esquerdo/direito selecionado | tratar como tabs ou toggle; `stateDescription` da opção ativa | esquerdo; direito |
| RecipeCard | Card de receita em listas/grade | `title`, `time: Duration?`, `livrosDeReceitas: List<String>`, `variant: List|Tile`, `onClick` | normal, loading, error | contentDescription no card; foco visível | List; Tile; loading; error |
...
| EmptyState | Vazio genérico para listas/telas | `illustration(slot)?`, `title`, `message`, `primaryAction?`, `secondaryAction?` | — | título como heading; ilustração decorativa quando apropriado | variações com/sem ilustração; 1 e 2 CTAs |
| ErrorState | Erro genérico com ações | `illustration(slot)?`, `title`, `message`, `primaryAction?`, `secondaryAction?` | — | contraste adequado; leitura clara do erro | genérico; específico |
